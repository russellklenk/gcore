...
static bool create_iobuf_allocator(iobuf_allocator_t *alloc, size_t total_size, size_t alloc_size)
{
    ...
    // VM_FLAGS_SUPERPAGE_SIZE_ANY specifies the equivalent of the MADV_HUGEPAGE.
    void  *baseaddr = mmap(NULL, total_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, VM_FLAGS_SUPERPAGE_SIZE_ANY, 0);
    if (baseaddr == NULL)
